...
basedir = "C:\\Research\\Kaggle\\AfricaSoil"
setwd(basedir)

# Read training data and add Group
train <- read.csv("./training.csv",header=TRUE,stringsAsFactors=FALSE)
train$train <- 1
groupings <- read.csv("./groupings_train.csv",header=TRUE,stringsAsFactors=FALSE)
groupings$Freq <- NULL
...
test$Sand <- 0.0
test$train <- 0
groupings <- read.csv("./groupings_test.csv",header=TRUE,stringsAsFactors=FALSE)
groupings$Freq <- NULL
groupings$TMAP <- round(groupings$TMAP,5)
