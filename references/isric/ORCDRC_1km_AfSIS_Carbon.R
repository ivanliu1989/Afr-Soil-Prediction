# title         : ORCDRC_1km_AfSIS.R
# purpose       : Mapping soil organic carbon for Africa (1 km resolution);
...
ov.afsp <- overlay(x=afgrid1km, y=afsp.geo, method="ORCDRC", var.type = "numeric")
names(ov.afsp)[which(names(ov.afsp)=="observedValue")] = "ORCDRC"
## Log-model:
formulaString.ORC <- as.formula(paste('log1p(ORCDRC) ~ DEMSRE3a + SLPSRT3a + TDMMOD3a + TDSMOD3a + TNMMOD3a + TNSMOD3a + EVMMOD3a + EVSMOD3a + PREGSM1a +', paste("G0", c(1:7,9), "ESA3a", sep="", collapse="+"), '+', paste("G", 10:21, "ESA3a", sep="", collapse="+"), '+ ns(altitude, df=2) + SGEUSG3x + TWISRE3a + L3POBI3a'))
## fit model:
ORC.m <- fit.regModel(formulaString.ORC, rmatrix=ov.afsp, afgrid1km, method="GLM", family=gaussian(), stepwise=TRUE, dimensions = "3D")
...
ORC.m@vgmModel$psill = c(0.32, 0.11)
v = ORC.m@vgmModel
...
fix.c = levels(afgrid1km$SGEUSG3x)[!(levels(afgrid1km$SGEUSG3x) %in% levels(ORC.m@regModel$model$SGEUSG3x))]
for(j in fix.c){
...
## 3D points:
observed <- SpatialPointsDataFrame(ORC.m@sp, data=data.frame(observedValue=ORC.m@regModel$model[,1]))
observed@data[,"residuals"] <- residuals(ORC.m@regModel)
#observed[1:3,]
## remove duplicates:
...
system.time(for(i in 1:6){
  if(is.na(file.info(paste("ORCDRC", "_sd", i, "_M.tif", sep=""))$size)){
  ...
  for(j in 1:L){  ## takes about 10 mins and > 8GB!
    rp <- stats::predict.glm(ORC.m@regModel, newdata=afgrid1km@data[sel[j]:sel[j+1],], type="response", se.fit = TRUE, na.action = na.pass)
    gc()
    ## copy values:
    rkgrid1km@data[sel[j]:sel[j+1], paste("ORCDRC", "_sd", i, "_glm", sep="")] <- rp$fit
    rkgrid1km@data[sel[j]:sel[j+1], paste("ORCDRC", "_sd", i, "_var", sep="")] <- rp$se.fit^2
    gc()
  }
  ...
  gc()
  rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_var", sep="")] <- rkgrid1km@data[,paste("ORCDRC", "_sd", i, "_var", sep="")] + rk$var1.var
  gc()  
  rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_M", sep="")] <- expm1(rkgrid1km@data[,paste("ORCDRC", "_sd", i, "_glm", sep="")] + rk$var1.pred + rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_var", sep="")]/2)
  gc()
  rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_L", sep="")] <- expm1(rkgrid1km@data[,paste("ORCDRC", "_sd", i, "_glm", sep="")] + rk$var1.pred - 1.645*sqrt(rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_var", sep="")]))
  gc()
  rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_U", sep="")] <- expm1(rkgrid1km@data[,paste("ORCDRC", "_sd", i, "_glm", sep="")] + rk$var1.pred + 1.645*sqrt(rkgrid1km@data[, paste("ORCDRC", "_sd", i, "_var", sep="")]))
  
  ## write to GeoTiff:
  writeGDAL(rkgrid1km[paste("ORCDRC", "_sd", i, "_M", sep="")], paste("ORCDRC", "_sd", i, "_M.tif", sep=""), "GTiFF", mvFlag=-9999, type="Int16")
  gc()
  writeGDAL(rkgrid1km[paste("ORCDRC", "_sd", i, "_L", sep="")], paste("ORCDRC", "_sd", i, "_L.tif", sep=""), "GTiFF", mvFlag=-9999, type="Int16")
  gc()
  writeGDAL(rkgrid1km[paste("ORCDRC", "_sd", i, "_U", sep="")], paste("ORCDRC", "_sd", i, "_U.tif", sep=""), "GTiFF", mvFlag=-9999, type="Int16")
  gc()
  #writeGDAL(rkgrid1km[paste("ORCDRC", "_sd", i, "_var", sep="")], paste("ORCDRC", "_sd", i, "_var.tif", sep=""), "GTiFF", mvFlag=-99999)
  #gc()
  }
})
...
system("7za a ORCDRC_1km_glmrk.tif.7z ORCDRC_sd*.tif")
## save the model:
save(ORC.m, file="ORC.m.rda", compress="xz")
## add a readme file:
#system("7za a ORCDRC_1km_glmrk.tif.7z README_ORCDRC.txt")
#save.image(file="ORCDRC_AfSIS.RData")
